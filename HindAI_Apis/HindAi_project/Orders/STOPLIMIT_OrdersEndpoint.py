...
from .orders_codes.BUY_STOPLIMIT_Order import BUY_stop_Order
from .orders_codes.SELL_STOPLIMIT_Order import SELL_stop_Order
from .orders_codes.Order_Status import get_order_status
from .orders_codes.cancel_order import cancel_order
from .models import StopLimitOrder
...
class StopLimitOrderResponse(BaseModel):
    success: bool
    message: str
...
class StopLimitOrderStatusRequest(BaseModel):
    username: str
    account_id: str

class StopLimitOrderStatusResponse(BaseModel):
    success: bool
    message: str
    orders: List[dict] = []
    updated_orders: int = 0

class CancelStopLimitOrderRequest(BaseModel):
    username: str
    account_id: str
    brokerage_order_id: str

class CancelStopLimitOrderResponse(BaseModel):
    success: bool
    message: str
    order_id: Optional[str] = None
    data: Optional[dict] = None
...
async def process_stoplimit_order(order_request: StopLimitOrderRequest, order_function, action_type: str):
    """Common function to process both BUY and SELL stop limit orders"""
    try:
        ...
        # Execute the Stop Limit Order using actual function
        try:
            ...
            return StopLimitOrderResponse(
                ...
                data={
                    ...
                    "suggestions": [
                        ...
                        "Verify limit and stop prices are reasonable",
                        "Ensure stop price is below limit price for BUY orders",
                        "Ensure stop price is above limit price for SELL orders"
        ...
        if response:
            ...
            # Create StopLimitOrder instance
            stoplimit_order_obj = await sync_to_async(StopLimitOrder.objects.create)(
                user=django_user,
            ...
            if universal_symbol:
                stoplimit_order_obj.universal_symbol_id = universal_symbol.get('id')
                stoplimit_order_obj.raw_symbol = universal_symbol.get('raw_symbol')
                stoplimit_order_obj.symbol_name = universal_symbol.get('symbol')
                stoplimit_order_obj.description = universal_symbol.get('description')
                stoplimit_order_obj.figi_code = universal_symbol.get('figi_code')
                stoplimit_order_obj.logo_url = universal_symbol.get('logo_url')
                
                # Extract FIGI instrument data
                figi_instrument = universal_symbol.get('figi_instrument', {})
                if figi_instrument:
                    stoplimit_order_obj.figi_share_class = figi_instrument.get('figi_share_class')
                
                # Extract exchange data
                exchange = universal_symbol.get('exchange', {})
                if exchange:
                    stoplimit_order_obj.exchange_id = exchange.get('id')
                    stoplimit_order_obj.exchange_code = exchange.get('code')
                    stoplimit_order_obj.exchange_name = exchange.get('name')
                    stoplimit_order_obj.exchange_mic_code = exchange.get('mic_code')
                    stoplimit_order_obj.exchange_timezone = exchange.get('timezone')
                    stoplimit_order_obj.exchange_suffix = exchange.get('suffix')
                    stoplimit_order_obj.exchange_start_time = exchange.get('start_time')
                    stoplimit_order_obj.exchange_close_time = exchange.get('close_time')
                
                # Extract currency data
                currency = universal_symbol.get('currency', {})
                if currency:
                    stoplimit_order_obj.currency_id = currency.get('id')
                    stoplimit_order_obj.currency_code = currency.get('code')
                    stoplimit_order_obj.currency_name = currency.get('name')
                
                # Extract security type data
                security_type = universal_symbol.get('type', {})
                if security_type:
                    stoplimit_order_obj.security_type_id = security_type.get('id')
                    stoplimit_order_obj.security_type_code = security_type.get('code')
                    stoplimit_order_obj.security_type_description = security_type.get('description')
                    stoplimit_order_obj.security_type_is_supported = security_type.get('is_supported', True)
            
            # Save the updated object
            await sync_to_async(stoplimit_order_obj.save)()
            
            return StopLimitOrderResponse(
                success=True,
                message=f"Stop Limit {action_type} order placed successfully",
            ...
            return StopLimitOrderResponse(
                success=False,
                message="Failed to place stop limit order - no response received",
        ...
        print(f"Error in {action_type} stop limit order: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error placing {action_type} stop limit order: {str(e)}")

@router.post("/stoplimit-order-BUY", response_model=StopLimitOrderResponse)
async def create_buy_stoplimit_order(order_request: StopLimitOrderRequest):
    return await process_stoplimit_order(order_request, BUY_stop_Order, "BUY")

@router.post("/stoplimit-order-SELL", response_model=StopLimitOrderResponse)
async def create_sell_stoplimit_order(order_request: StopLimitOrderRequest):
    return await process_stoplimit_order(order_request, SELL_stop_Order, "SELL")

@router.post("/cancel-stoplimit-order", response_model=CancelStopLimitOrderResponse)
async def cancel_stoplimit_order_endpoint(cancel_request: CancelStopLimitOrderRequest):
    try:
        # Get Django user by username
        try:
        ...
        try:
            existing_order = await sync_to_async(StopLimitOrder.objects.get)(
                brokerage_order_id=cancel_request.brokerage_order_id,
                user=django_user
            )
        except StopLimitOrder.DoesNotExist:
            raise HTTPException(status_code=404, detail="Stop Limit order not found in database")
        
        # Check if order is already canceled or executed
        if existing_order.status in ['CANCELLED', 'CANCELED', 'EXECUTED', 'FILLED']:
            return CancelStopLimitOrderResponse(
                success=False,
        ...
        print(f"Error in cancel_stoplimit_order: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error cancelling stop limit order: {str(e)}")

@router.post("/stoplimit-order-status", response_model=StopLimitOrderStatusResponse)
async def get_and_update_stoplimit_order_status(status_request: StopLimitOrderStatusRequest):
    try:
        # Get Django user by username
        try:
        ...
        if not api_orders:
            return StopLimitOrderStatusResponse(
                success=True,
                message="No stop limit orders found",
                orders=[],
                updated_orders=0
            )
        ...
        # Filter for stop limit orders and process each one
        for api_order in api_orders:
            # Check if this is a stop limit order (has both limit_price and stop_price and order_type is StopLimit)
            if (api_order.get('limit_price') is None or 
                api_order.get('stop_price') is None or 
                api_order.get('order_type') != 'StopLimit'):
                continue
            ...
            if brokerage_order_id:
                try:
                    # Find existing order in database for this specific user
                    existing_order = await sync_to_async(
                        StopLimitOrder.objects.get
                    ...
                    # Order not in database for this user, check if it exists for any user
                    try:
                        # Check if order exists for any user (to avoid UNIQUE constraint violation)
                        existing_any_user = await sync_to_async(
                            StopLimitOrder.objects.filter(brokerage_order_id=brokerage_order_id).exists
                        )()
                        
                        if existing_any_user:
                            # Order exists for another user, skip creation
                            print(f"Order {brokerage_order_id} already exists for another user, skipping...")
                            continue
                        ...
                        new_order = await sync_to_async(StopLimitOrder.objects.create)(
                            user=django_user,
                        ...
                        print(f"Failed to create order {brokerage_order_id}: {str(create_error)}")
                        continue
        ...
        return StopLimitOrderStatusResponse(
            success=True,
            message=f"Successfully fetched and updated {updated_count} stop limit orders",
            orders=orders_data,
            updated_orders=updated_count
        )
        
    except Exception as e:
        print(f"Error in get_and_update_stoplimit_order_status: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error fetching stop limit order status: {str(e)}")

@router.get("/stoplimit-orders/{username}")
async def get_user_stoplimit_orders(username: str):
    try:
        # Get Django user by username
        try:
        ...
        orders = await sync_to_async(list)(
            StopLimitOrder.objects.filter(user=django_user).order_by('-time_placed')
        )
        orders_data = []
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching stop limit orders: {str(e)}")